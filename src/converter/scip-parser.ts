...
import { Index, IndexSchema, Document, Occurrence, SymbolInformation, SymbolRole } from "./scip_pb.js";
...
export async function parseScipFile(scipPath: string): Promise<ScipData> {
  ...
  // Parse documents
  const documents: ParsedDocument[] = index.documents.map(doc => parseDocument(doc));

  // Parse external symbols
  const externalSymbols: ParsedSymbol[] = index.externalSymbols.map(sym => parseSymbol(sym));
...
function parseOccurrence(occ: Occurrence): ParsedOccurrence {
  ...
  if (occ.enclosingRange && occ.enclosingRange.length > 0) {
    if (occ.enclosingRange.length === 3) {
      enclosingRange = [occ.enclosingRange[0], occ.enclosingRange[1], occ.enclosingRange[0], occ.enclosingRange[2]];
    } else if (occ.enclosingRange.length === 4) {
      enclosingRange = [occ.enclosingRange[0], occ.enclosingRange[1], occ.enclosingRange[2], occ.enclosingRange[3]];
    }
  }
...
export function buildLookupMaps(scipData: ScipData): {
  ...
  const definitionsBySymbol = new Map<string, { file: string; definition: SymbolDefinition }>();
  ...
  for (const doc of scipData.documents) {
    ...
    for (const def of definitions) {
      definitionsBySymbol.set(def.symbol, { file: doc.relativePath, definition: def });
    }
  }
 ...
 */
export function getDocumentSymbols(doc: ParsedDocument, symbolsById: Map<string, ParsedSymbol>): Array<{
  symbol: string;
  name?: string;
  kind: number;
  range: [number, number, number, number];
