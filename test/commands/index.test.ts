...
describe("index command - database conversion", () => {
	...
	// They use the existing dora database for verification

	test("should create database with correct schema", () => {
		// Skip if database doesn't exist
		if (!existsSync(existingDbPath)) {
			console.log(
				"Skipping test: .dora/dora.db not found. Run 'dora index' first.",
			);
			return;
		}
	...
	test("should have files table with correct columns", () => {
		...
		const filesColumns = db.query("PRAGMA table_info(files)").all() as Array<{
			name: string;
		}>;
